...
export const mockApiClient: ApiClient = {
  ...
  async register(data) {
    ...
    if (db.users.find((u) => u.name === data.name)) {
      throw new Error('Name taken (mock)');
    }
  ...
  async getBMI(token) {
    ...
    const user = db.users.find((u) => u.id === uid);
    if (!user || !user.weightKg || !user.heightM) {
      throw new Error('User data incomplete (mock)');
    }
  ...
  async updateWater(token, id, datetime, amountMl) {
    ...
    if (idx === -1) {
      throw new Error('Record not found (mock)');
    }
    const old = db.water[uid][idx];
    const updatedWater: WaterRecord = {
      id,
      datetime: datetime ?? old.datetime,
      amountMl: amountMl ?? old.amountMl,
    };
  ...
  async updateSleep(token, id, datetime, hours) {
    ...
    if (idx === -1) {
      throw new Error('Sleep record not found (mock)');
    }
    const old = db.sleep[uid][idx];
    const updatedSleep: SleepRecord = {
      id,
      datetime: datetime ?? old.datetime,
  ...
  async updateActivity(token, id, datetime, minutes, intensity) {
    ...
    if (idx === -1) {
      throw new Error('Activity record not found (mock)');
    }
    const old = db.activity[uid][idx];
    const updatedAct: ActivityRecord = {
      id,
      datetime: datetime ?? old.datetime,
  ...
  async updateCustomItem(_token, categoryId, itemId, datetime, note) {
    ...
    if (idx === -1) {
      throw new Error('Custom item not found (mock)');
    }
  ...
  async deleteCustomCategory(_token, categoryId) {
    ...
    db.customCategories = (db.customCategories || []).filter(
      (c) => c.id !== categoryId && c.categoryName !== categoryId
    );
    if (db.customData) {
      delete db.customData[categoryId];
    }
  },
};
